...
from models import DadosCondominio, ValidationStatus, ExtractionLayer
    ...
    def validate_and_fallback(self, 
        ...
        for bloco in dados.blocos:
            for apt in bloco.apartamentos:
                ...
                # DIVERGENTE - Tenta fallback pdfplumber nas páginas do apto
                logger.warning(f"Divergência {div:.1%} em {apt.apartamento} (Pág {apt.pagina_origem}). Acionando FALLBACK...")
                ...
                logger.error(f"Fallback falhou para {apt.apartamento}. Soma: {apt.soma_calculada:.2f} | Total: {apt.total_impresso:.2f}")
